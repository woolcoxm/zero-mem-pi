...
## Remaining roadmap
- **v0.10** ✅ End-to-end paper-metric eval (`eval-reader.ts`): LLM-reader token-F1 / exact-match / BLEU-1 on real LoCoMo10 — retrieve top-K (coverage), LLM answers at temp 0, score vs gold; per-category; degrades to retrieval hit-rate when no endpoint. **Determinism verified**: embeddings are bit-exact across runs (single-thread ONNX, drift 0) — the run-to-run "variance" seen during v0.9 tuning was a harness/env-propagation bug, not the model. evals now take CLI args; LLM uses temp 0.
- **v0.11** Beat BM25 outright on LoCoMo: a stronger/conversational embedder (bge-small lifts pure-semantic 0.27→0.42 — built, pending a validated re-do after the load-bug incident) or a cross-encoder reranker over the coverage candidate set. (Coverage currently *ties* BM25; pure dense loses; in-core single-pass RRF and the ms-marco reranker were tested but did NOT beat BM25.)
