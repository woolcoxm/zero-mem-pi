...
- **v0.11** **min-max per-view normalization** (Eq 12: `(S−S_min)/(S_max−S_min)`) — we use max-norm; match the paper's fusion exactly + re-validate.
- **v0.11** **evidence-level calibration** (Filter provenance/boundary + Rank by subject/temporal/answer-type compatibility, Eq 15) — currently answer-level type checks only.
- **v0.11** eval at **top-10** retrieval budget (paper's ablation: best avg) + a larger reader sample for stabler end-to-end F1.
- **v0.11** Incremental-HNSW graph-quality refresh tuning (insertion-count vs recall drift).
