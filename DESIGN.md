...
## Remaining roadmap
- **v0.9** True paper eval: run against real LoCoMo with an LLM reader for end-to-end F1/BLEU (needs dataset + model; `eval.ts` covers the deterministic retrieval view meanwhile).
- **v0.9** HNSW incremental insert (add-time graph updates) instead of growth-gated full rebuilds.
- **v0.9** Adaptive MMR λ (lower for entity-specific lookups, higher for exploratory queries).
- **v0.9** Cross-project federation when `scopeToProject` is too narrow.
