...
- **v0.11** Beat BM25 outright on LoCoMo: coverage currently *ties* BM25 (0.534 vs 0.529); bge lifted pure-dense to 0.418 but it still loses. A cross-encoder reranker over the coverage candidate set, or a conversational/embedder fine-tuned for dialogue. (In-core single-pass RRF and the ms-marco reranker were tested but did NOT beat BM25; a two-pass RRF reached 0.550 but isn't single-pass-realizable.)
- **v0.11** Incremental-HNSW graph-quality refresh tuning (insertion-count vs recall drift) now that add-time inserts are live.
